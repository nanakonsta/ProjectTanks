...
class TankFactory:
    ...
    def constructTank(self, tankType, tank_pos_x, tank_pos_y, distanceTostop = 0, fireRate = 200, health = 100, gameManager = None, p1orp2 = ""):
        match tankType:
            case tankType.P1:
                p1 = TankBase(tank_pos_x,tank_pos_y,self.p1BasePath,2, 3, tankType.P1, 0, health, gameManager, p1orp2)
                tank_turret = TankTurret(p1, self.p1TopPath, fireRate)
                p1.tank_turret = tank_turret
                p1.add_child(tank_turret)
                return p1  
            case tankType.P2:
                p2 = TankBase(tank_pos_x,tank_pos_y,self.p2BasePath,2, 3,tankType.P2, 0, health, gameManager, p1orp2)
                tank_turret = TankTurret(p2, self.p2TopPath, fireRate)
                p2.tank_turret = tank_turret
                p2.add_child(tank_turret)
                return p2  
            case tankType.E1:
                e1 = TankBase(tank_pos_x,tank_pos_y,self.e1BasePath,2, 3, tankType.E1, distanceTostop, health, gameManager, p1orp2)
            ...
            case tankType.E2:
                e2 = TankBase(tank_pos_x,tank_pos_y,self.e2BasePath,2, 3, tankType.E2,distanceTostop, health, gameManager, p1orp2)
